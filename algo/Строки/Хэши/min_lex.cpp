...
static const int MAX_LENGTH = 2e5;
...
public:
    ...
    static bool is_equal(const hstr &s1, int l1, int r1, const hstr &s2, int l2, int r2){ // [l, r)
    ...
    static int max_pref(const hstr &s1, int p1, const hstr &s2, int p2, int maxlen = -1){
        int l = 0;
     ...
     * 1    ->   s1(l1,r1) > s2(l2, r2)
     */
    static int compare(const hstr &s1, int l1, int r1, const hstr &s2, int l2, int r2) {
    ...
    static int compare(const hstr &s1, const hstr &s2) {
        return compare(s1, 0, s1.size, s2, 0, s2.size);
    }

    friend istream& operator>>(istream &stream, hstr &hs) {
        stream >> hs.s;
        hs.self_init();
        return stream;
    }
    friend ostream& operator<<(ostream &stream, const hstr &hs) { return stream << hs.s; }

    bool operator<(const hstr &hs) const { return compare(*this, hs) == -1; }
    bool operator==(const hstr &hs) const { return compare(*this, hs) == 0; }
    bool operator!=(const hstr &hs) const { return compare(*this, hs) != 0; }
    bool operator>(const hstr &hs) const { return compare(*this, hs) == 1; }
};
...
int main() // O(n log n)
{
    ...
    int mini = 0;
    for (int i = 1; i < n; i++){
        if (hstr::compare(hs, i, i + n, hs, mini, mini + n) == -1)
            mini = i;
    }
    println(s.substr(mini, n));
    return 0;
}
